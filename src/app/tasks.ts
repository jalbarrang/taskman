...
function taskStatus(status: string): TaskStatus {
  ...
  throw new AppError('INVALID_INPUT', `Invalid status "${status}". Use one of: ${TASK_STATUSES.join(', ')}.`, {
    status,
    valid: TASK_STATUSES,
  });
}
...
export async function updateTask(
  ...
  return { planName, taskId: result.task.id, status: result.task.status, finalizable: result.finalizable };
}
...
export async function addDeferredTask(
  ...
  if (!reason) throw new AppError('INVALID_INPUT', '--reason is required (why the follow-up matters).');
  const { planName, planDir } = await requirePlan(context, input.plan);
