...
const meta = (name: string) => ({ _type: 'meta' as const, title: name, plan_name: name, created_at: 'now' });
const task = { _type: 'task' as const, id: 't-001', description: 'read', status: 'pending' as const, created_at: 'now', updated_at: 'now' };
...
describe('application status and root binding', () => {
  test('binds a configured root and reads a plan from it', async () => {
    ...
    expect(context).toMatchObject({ root: join(dir, 'ledger'), displayRoot: 'ledger', source: 'taskmanrc' });
    ...
    expect(status).toMatchObject({ planName: 'Only', counts: { pending: 1 }, finalizable: false, handoff: '# Handoff' });
  });
  ...
  test('returns read candidates and rejects an ambiguous strict resolution', async () => {
    ...
    await expect(requirePlan(context)).rejects.toMatchObject({ code: 'AMBIGUOUS_PLAN' } satisfies Partial<AppError>);
  });
});
